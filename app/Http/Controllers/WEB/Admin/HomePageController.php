...
class HomePageController extends Controller
{
    ...
    {
        $pcBuilders = PcBuilder::with('category', 'sub_category')->get();
        $categories = Category::where("status", 1)->get();
        $subCategories = SubCategory::where("status", 1)->get();
        $childCategories = ChildCategory::where("status", 1)->get();
        $allCategories = $categories->merge($subCategories)->merge($childCategories);
        $banner = Setting::select("popular_category_banner")->first();

        return view("admin.pc_builder_page", compact("pcBuilders", "allCategories", "banner"));
    }
    
    public function edit_pcbuilders($id){
        $data = PcBuilder::find($id);
        $categories = Category::where("status", 1)->get();
        $subCategories = SubCategory::where("status", 1)->get();
        $childCategories = ChildCategory::where("status", 1)->get();
        $allCategories = $categories->merge($subCategories)->merge($childCategories);
        $edit_data = view('admin.edit_pcbuilder',compact('data','allCategories'))->render();
    ...
    public function storePcBuild(Request $request)
    {
        ...
        PcBuilder::create([
            ...
            'is_required' => ($request->is_required == 'on') ? '1' : null
        ]);
    ...
    public function updatePcBuild(Request $request){
        PcBuilder::where('id', $request->id)->update([
            'category_id' => $request->category_id, 
            'serial'      => $request->serial,
            'is_required' => ($request->is_required == 'on') ? '1' : null
        ]);
