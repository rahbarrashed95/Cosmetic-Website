...
class OrderController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $query = Order::with('user', 'assign', 'orderProducts.Product')->whereHas('orderProducts.product', function($q) {
           $q->whereNotNull('name');
        });
    ...
    public function addNewComboProductEntryadd(Request $request){
        ...
        if($variation->type == 'variable') {
          ...
          $html = view('admin.orders.add_var_combo_product_row', compact('variation','pr_id'))->render();  
            ...
            foreach ($request->product_id as $key => $product_id) {
               ...
               if(isset($request->order_product_id[$key])) {
                ...
                if($ord_qty < $pro_qty) {
                    ...
                    $product_id = $request->product_id[$key];
                    $quantity = $request->quantity[$key];
                    $stockCheck = ProductStock::where('color_id', '1')->where('size_id', '1')->where('product_id', $product_id)->first();
