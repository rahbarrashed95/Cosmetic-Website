...
class ProductController extends Controller
{
    public function index(Request $request)
    {
      if(!auth()->user()->can('product.index')){
        ...
        $query = Product::with('category', 'seller', 'brand','offerProduct');
    ...
    public function store(Request $request) {
        ...
        if($request->thumb_image){
            ...
            $destation_path1 = 'uploads/product/thumb-big-image/'.$image_name;
            $image->resize(800,800);
            $image->save(public_path().'/'.$destation_path1);

            // For Main Image
            $destation_path = 'uploads/product/thumb-small-image/'.$image_name;
            $image->resize(480,480);
            $image->save(public_path().'/'.$destation_path);
            $product->thumb_image=$image_name;
        }
        
        if($request->youtube_thumb){
            
            $image = Image::make($request->file('youtube_thumb'));
            $extention = $request->youtube_thumb->getClientOriginalExtension();
            $image_name = Str::slug($request->name).date('-Y-m-d-h-i-s-').rand(999,9999).'.'.$extention;

            // For Main Image
            $destation_path = 'uploads/product/youtube-thumbnails/'.$image_name;
            $image->resize(50,50);
            $image->save(public_path().'/'.$destation_path);
            $product->youtube_thumb=$image_name;
        }
        ...
        $product->wholesell_price_discount = $request->wholesell_price_discount;
        ...
        if ($request->hasFile('images')) {
            ...
            foreach ($request->file('images') as $key => $image) {
                ...
                $destation_path_another = 'uploads/product/multi-big-images/'.$image_name;
                $image->resize(800,800);
        ...
        if ($request->type == 'variable') {
            ...
            foreach ($colors as $key => $colorId) {
                ...
                if (isset($images[$key])) {
                    ...
                    $imageName = Str::slug($product->name) . '_color_' . $colorId . date('-Y-m-d-h-i-s-') . rand(999, 9999) . '.' . $ext;
                    $destinationPath = 'uploads/product/variation-images/' . $imageName;
      ...
      //dd($request->all());
        ...
        $this->validate($request, $rules,$customMessages);
         ...
         if($request->thumb_image){
            ...
            $destation_path2 = 'uploads/product/thumb-small-image/'.$image_name;
            $image->resize(480,480);
        ...
        if($request->youtube_thumb){
            $old_youtube = $product->youtube_thumb;
            $image = Image::make($request->file('youtube_thumb'));
            $extention = $request->youtube_thumb->getClientOriginalExtension();
            $image_name = Str::slug($request->name).date('-Y-m-d-h-i-s-').rand(999,9999).'.'.$extention;
            ...
            if($old_youtube){
                if(File::exists(public_path().'/uploads/custom-images2/'.$old_youtube))unlink(public_path().'/uploads/custom-images2/'.$old_youtube);
                if(File::exists(public_path().'/uploads/main-image/'.$old_youtube))unlink(public_path().'/uploads/main-image/'.$old_youtube);
            }
        ...
        if($product->type != 'single'){
            $delete_variations = ProductVariant::where('product_id', $product->id)->whereNotIn('id',  $request->color_variation_id)->get();
            ...
            // Initialize the array to collect new variations
          
            foreach ($request->color_id as $key => $color) {
                // dd($request->color_id);
                
                // Check if the current request has an image and color variation ID
                if (isset($request->color_variation_id[$key])) {
                    $product_variant = ProductVariant::find($request->color_variation_id[$key]);
            
                    if (!$product_variant) {
                        // Handle the case where the ProductVariant is not found
                        continue;
                    }
                    ...
                    if (isset($images[$key])) {
                        $ext = $images[$key]->getClientOriginalExtension();
                        $imageName = Str::slug($product->name) . '_color_' . $color . date('-Y-m-d-h-i-s-') . rand(999, 9999) . '.' . $ext;
                        $destinationPath = 'uploads/custom-images/' . $imageName;
                        $image = Image::make($images[$key]);
                        $image->resize(800, 800);
                        $image->save(public_path() . '/' . $destinationPath);
            
                        $updateData['var_image'] = $destinationPath;
                    }
            ...
            // Create new product variations if there are any
            if (!empty($variationsData)) {
                $product->variations()->createMany($variationsData);
            }
        }
    ...
    public function destroy($id)
    {
      if(!auth()->user()->can('product.destroy')){
        ...
        if($old_thumbnail){
            ...
            if(File::exists(public_path().'/uploads/product/thumb-big-image/'.$old_thumbnail))unlink(public_path().'/uploads/product/thumb-big-image/'.$old_thumbnail);
        }
        ...
        foreach($gallery as $image){
            ...
            $image->delete();
            if($old_image){
                if(File::exists(public_path().'/uploads/product/thumb-big-images/'.$old_image))unlink(public_path().'/uploads/product/thumb-big-images/'.$old_image);
            }
        }
        
        foreach($variation as $vimage){
            $old_vimage = $vimage->var_image;
            $vimage->delete();
            if($old_vimage){
                if(File::exists(public_path().'/uploads/product/variation-images/'.$old_vimage))unlink(public_path().'/uploads/product/variation-images/'.$old_vimage);
            }
        }
    ...
    // public function change_popular($id){
    //   if(!auth()->user()->can('product.changeStatus')){
    //         abort(403, 'Unauthorized action.');
    //     }
    //     $product = Product::find($id);
