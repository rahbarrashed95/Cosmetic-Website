...
class AuthController extends Controller
{
    ...
    public function userCredentials() {
        $user = auth()->user();
        $user_credentials = view('frontend.profile.user_credentials', compact('user'))->render();
        return response()->json([
            'status' => true,
    ...
    public function print_my_invoice(Request $request, $id){
        
        $item=Order::with('user','orderProducts.orderProductVariants','orderAddress')->where('id', $id)->first();
        
        $setting = Setting::first();
        $view = view('frontend.order.invoice', compact('item', 'setting'))->render();
        return response()->json(['status'=> true, 'item'=> $item, 'setting'=> $setting,'view'=>$view]);
    }
    ...
    public function updateProfile(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image_name = Str::slug($data['name']) . date('-Y-m-d-h-i-s-') . rand(999, 9999) . '.' . $extension;
            $image_path = 'uploads/user/image/' . $image_name;
