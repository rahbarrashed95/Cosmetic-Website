...
<style>
    ...
    nav {
          ...
          <div class="section-body">
            ...
            <div class="row">
                <div class="col-md-4">
                    ...
                    <a href="{{ route('admin.deleteAllProduct')}}" class="btn btn-danger multi_product_delete"><i class="fas fa-close"></i> Delete All</a>
                            ...
                            <tbody>
                                @foreach ($products as $index => $product)
                                    <tr>
                                        ...
                                        <td>
                                        <a href="{{ route('admin.select_offer',$product->id) }}" class="btn btn-success btn-sm offer_btn"><i class="fa fa-gift" aria-hidden="true"></i></a>
                                        ...
                                        <td> <img class="rounded-circle" src="{{ asset('uploads/product/thumb-small-image/'.$product->thumb_image) }}" alt="" width="100px" height="100px"></td>
                                        <td>
                                            {{ $product->type }}
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            @if($product->offerProduct) 
                                                {{ $product->offerProduct->offer->name }} 
                                            @else
                                            @endif
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <a href="javascript:;" onclick="changePopular({{ $product->id }})">
                                                <input id="status_toggle" type="checkbox" checked data-toggle="toggle" data-on="{{__('admin.Active')}}" data-off="{{__('admin.InActive')}}" data-onstyle="success" data-offstyle="danger">
                                            </a>
      ...
      <div class="modal fade" id="offer_div" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
...
<script>
    ...
    $(document).on('click', 'a.offer_btn',function(e){
        e.preventDefault();
        ...
        $.ajax({
            ...
            success : function(res) {
                if(res.success){
                    $('div#offer_div').html(res.html).modal('show');
                }
            }
        });
    });
    ...
    function changePopular(id){
        ...
        $.ajax({
            type:"put",
            data: { _token : '{{ csrf_token() }}' },
            url:"{{url('/admin/product-popular-change/')}}"+"/"+id,
            success:function(response){
                toastr.success(response)
            },
            error:function(err){
